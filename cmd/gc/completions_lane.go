...
// completed a full traversal, advances the cadence, returns the whole sweep's
// totals, and clears only the force generation present when the traversal
// started. A later force remains latched so a fresh pass revisits roots the
// in-progress cursor may already have passed.
