...
func TestCompletionsLaneForceDuringSweepStartsFreshTraversal(t *testing.T) {
	lane := newCompletionsLane()
	now := time.Now()
	if _, done := lane.noteSweepChunk(now, backstopReasonStartup, 0, 1, 0, false); done {
		t.Fatal("first chunk unexpectedly completed the traversal")
	}
	lane.force()
	if _, done := lane.noteSweepChunk(now.Add(time.Second), backstopReasonStartup, 0, 1, 0, true); !done {
	...
	if _, done := lane.noteSweepChunk(now.Add(3*time.Second), backstopReasonCursorGap, 0, 2, 0, true); !done {
