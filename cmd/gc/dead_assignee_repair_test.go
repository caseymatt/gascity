...
func TestReleaseOrphanedPoolAssignments_SkipsLiveAssigneeStaysAssigned(t *testing.T) {
	...
	// nil sessions store is accepted here, but do not read this as coverage of
	// the sessionsStore==nil fallback: the assignee IS the live session's
	// session_name, so openSessionOwnsWork matches off the snapshot and skips
	// the bead before liveOpenSessionAssignmentExists is ever consulted.
	// TestReleaseOrphanedPoolAssignments_NilSessionsStoreFallsBackToWorkStore
	// is the test that reaches that gate.
